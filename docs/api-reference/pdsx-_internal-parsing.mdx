---
title: parsing
sidebarTitle: parsing
---

# `pdsx._internal.parsing`


argument parsing utilities.

## Functions

### `parse_key_value_args` <sup><a href="https://github.com/zzstoatzz/pdsx/blob/main/src/pdsx/_internal/parsing.py#L13" target="_blank"><Icon icon="github" style="width: 14px; height: 14px;" /></a></sup>

```python
parse_key_value_args(args: list[str]) -> dict[str, Any]
```


parse key=value arguments into a dict.

**Args:**
- `args`: list of key=value strings

**Returns:**
- parsed dictionary

**Examples:**

>>> parse_key_value_args(["name=test", "count=5", "active=true"])
{'name': 'test', 'count': 5, 'active': True}

